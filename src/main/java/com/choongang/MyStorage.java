...
public class MyStorage {
    ...
    static void prod_input(Scanner s) {
        ...
        if (productCount >= products.length) {
            System.out.println("[System] 제품 등록이 불가능합니다. 배열이 가득 찼습니다.");
            return;
        }
        ...
        System.out.println("[System] 제품 '" + product + "' 이(가) 성공적으로 등록되었습니다.");
    }
    ...
    static void prod_remove(Scanner s) {
        ...
        for (int i = 0; i < products.length; i++) {
            if (products[i] != null && products[i].equals(productName)) {
                products[i] = EMPTY; // 제품명 초기화
                counts[i] = 0; // 수량 초기화
                int productCount = 0;
                productCount--;
                System.out.println("[System] 제품 '" + productName + "' 의 등록이 취소되었습니다.");
                return;
            }
        }
        System.out.println("[System] '" + productName + "' 이(가) 등록된 제품 목록에 없습니다.");
    }
